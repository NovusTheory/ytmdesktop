...
const enhancedMediaService = new EnhancedMediaService();
...
const store = new Conf<StoreSchema>({
  ...
  defaults: {
    ...
    integrations: {
      ...
      lastFMEnabled: false,
      enhancedMediaServiceEnabled: false
    },
    shortcuts: {
      playPause: "",
      next: "",
...
store.onDidAnyChange(async (newState, oldState) => {
  ...
  if (newState.integrations.enhancedMediaServiceEnabled) {
    enhancedMediaService.provide(ytmView);
  }
  if (newState.integrations.enhancedMediaServiceEnabled && !oldState.integrations.enhancedMediaServiceEnabled) {
    enhancedMediaService.enable();
    log.info("Integration enabled: Enhanced media service");
  } else if (!newState.integrations.enhancedMediaServiceEnabled && oldState.integrations.enhancedMediaServiceEnabled) {
    enhancedMediaService.enable();
    log.info("Integration disabled: Enhanced media service");
  }
...
if (store.get("integrations").enhancedMediaServiceEnabled) {
